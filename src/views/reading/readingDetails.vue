        ...
        <div class="cont-a">
          ...
          <div class="right-c">
            <div class="right">
              ...
              <p>价格：{{movieData.price}}</p>
              <p v-show="movieData.publisher">出版社：{{movieData.publisher}}</p>
              <p v-show="movieData.pubdate">出版年：{{movieData.pubdate}}</p>
              <p v-show="movieData.binding">装帧: {{movieData.binding}}</p>
            </div>
          </div>
        </div>
      </div>
...
<script type="text/ecmascript-6">
  export default {
    ...
    methods: {
      ...
      // 获取数据
      getDetails () {
        this.$fetch('/' + this.$route.query.id, {}, 'book').then(res => {
          if (res.title) {
            this.movieData = res
          }
        })
